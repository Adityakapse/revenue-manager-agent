...
create table if not exists public.reservations_hackathon (
  ...
  -- NOTE (solution decision): the data site's reservations use a large, open set of
  -- granular commercial rate codes (e.g. OCHEARLY, BOOKPROM) that intentionally exceed
  -- the 8 canonical plans in rate_plan_lookup. The site itself does not enforce this FK.
  -- We keep rate_plan_lookup reconciled to /verify (8 rows) and preserve each reservation's
  -- true rate_plan_code, so this column is plain text (NOT a FK). See ARCHITECTURE.md.
